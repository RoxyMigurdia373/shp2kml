...
## 批量上传与下载
上传控件支持多选 ZIP，每个压缩包上限 1 GB。批量开始转换后，提供每个压缩包的结果下载及全部结果 ZIP。重复文件名用编号区分，损坏压缩包不影响其他文件。更改输入、编码、CRS 或输出格式后需重新转换。多文件会累计占用内存，云端建议分批上传。
批量回归测试：python test_batch_ui.py
